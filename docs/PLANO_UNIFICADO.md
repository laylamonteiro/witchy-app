...
| ~~Impressão digital do Google~~ **FECHADO 23/08** | — | A dona confirmou: o login Google **funciona no app publicado pela Play** — logo a impressão digital de app signing JÁ está registrada no projeto `grimorio-de-bolso` (o "already in use" que ela viu ao tentar registrar era a prova disso, não um problema). O registro que vale mora no painel do Google, não no repo: o `google-services.json` daqui é um download antigo (só o hash de debug) e o app não lê esse hash em runtime — usa o client id do código. Nada a fazer; opcional de higiene: re-baixar o json do console para o repo refletir o painel. O `SHA1_ESPERADO` do release.yml confere com a chave de UPLOAD (releases passam na guarda), também nada a mudar |
