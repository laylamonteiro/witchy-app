...
CREATE TABLE IF NOT EXISTS beta_codes (
  ...
  used_by TEXT,  -- Armazena ID do último usuário que usou o código (ou JSON array)
  used_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  -- Novos campos para múltiplos usos
  max_uses INTEGER NOT NULL DEFAULT 1,  -- Número máximo de usos (1 = uso único)
  current_uses INTEGER NOT NULL DEFAULT 0  -- Quantas vezes já foi usado
);
...
COMMENT ON COLUMN beta_codes.is_used IS 'Indica se o código esgotou todos os usos (current_uses >= max_uses)';
COMMENT ON COLUMN beta_codes.used_by IS 'ID do último usuário que resgatou o código';
COMMENT ON COLUMN beta_codes.used_at IS 'Data/hora do último resgate';
COMMENT ON COLUMN beta_codes.created_at IS 'Data/hora de criação do código';
COMMENT ON COLUMN beta_codes.max_uses IS 'Número máximo de usos permitidos (1 = uso único)';
