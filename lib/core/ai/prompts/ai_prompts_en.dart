...
- $_preservationEn''',
  ...
  encyIdentifySystemPrompt:
      'You are a visual identification expert for a witchcraft app. '
      'Examine the photo carefully BEFORE naming anything: note the shape, colors, '
      'leaves, arrangement and any distinctive trait of the herb or plant. '
...
- dreams (with meaning=interpretation), gratitudes, desires (with excerpt/evolution), affirmations (phrases she created or favorited), sigils (the INTENTION she drew), oracle (rune/pendulum/oracle/tarot draws, with question and answer), savedReadings (her readings — tarot, runes, pendulum, oracle, palmistry — with date, type and an excerpt of the whole reading: the question, the draw and the answer), freeWriting (reflections), practice (rites, spells created with name/purpose, notes), sky (the period's transits and phases).
...
- If a "menstrual" block is present, it is what SHE recorded of her own cycle and AUTHORIZED for this reading. Always cite it with attribution ("she recorded", "she chose"), never as a conclusion of the app. Anything under `not_recorded` was NOT recorded — that never becomes "no symptoms". Anything under `chosen_by_her` is her symbolic choice (the season, for instance), not a detected bodily phase, and `moon_estimated` is the app’s estimate. NEVER infer hormones, ovulation, fertility, pregnancy or a diagnosis; never say the cycle caused an event or that the Moon caused a symptom; do not treat pain, flow or any clinical condition; and do not repeat intimate details in every chapter — use them where the data sustains a relation.
