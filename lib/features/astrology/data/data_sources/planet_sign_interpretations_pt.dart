...
/// Interpretações mágicas planeta-signo — conteúdo em português (idioma-base).
///
/// As CHAVES do mapa (`'<planeta>_<signo>'`, derivadas de `Planet.name` e
/// `ZodiacSign.name`) são invariantes entre idiomas; apenas os VALORES de
/// texto são traduzidos. Mantenha as mesmas chaves nos três arquivos
/// (`planet_sign_interpretations_pt/en/es.dart`) — a paridade é verificada em
/// `test/astrology_interpretations_parity_test.dart`.
const Map<String, String> planetSignInterpretationsPt = {
  // SOL em cada signo
  'sun_aries':
...
/// Interpretação padrão (composta) caso não exista uma específica no mapa —
/// cobre planetas externos, nodos e os pontos místicos (MC/IC/DSC, Vértex,
/// Lilith, Parte da Fortuna). Junta o tema do planeta com o estilo do signo e
/// uma dica mágica.
String planetSignDefaultInterpretationPt(Planet planet, ZodiacSign sign) {
  final style = _signStylePt(sign);
  final magic = _planetMagicPt(planet);
  ...
  final article = (planet == Planet.lilith || planet == Planet.partOfFortune)
      ? 'Sua'
      : 'Seu';
  return '$article ${planet.displayName} em ${sign.displayName} vive '
      '${_planetMeaningPt(planet)}, $style. $magic';
}

/// Tema central de cada planeta (frase nominal).
String _planetMeaningPt(Planet planet) {
  switch (planet) {
    case Planet.sun:
...
String _signStylePt(ZodiacSign sign) {
  switch (sign) {
...
String _planetMagicPt(Planet planet) {
  switch (planet) {
    case Planet.sun:
...
/// Resumo curto para exibição em lista.
String planetSignShortInterpretationPt(Planet planet, ZodiacSign sign) {
  switch (planet) {
    case Planet.sun:
