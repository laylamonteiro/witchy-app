...
- O registro do Ciclo Menstrual é dado de saúde e tem regra própria: ele fica no aparelho e a sincronização geral não o leva. Só sai daqui se você ligar o envio do Ciclo Menstrual, um sim à parte, em Configurações → Privacidade — e as páginas que cada dia ganha no seu Grimório não saem nem assim.
...
- **São dois sins, e os dois nascem desligados.** O primeiro autoriza **registrar**: a roda do Ciclo Menstrual explica o que faz e o que fica onde antes de aceitar qualquer registro. O segundo, e só ele, autoriza **guardar o registro na nuvem da sua conta**, e fica em Configurações → Privacidade, ao lado das outras decisões sobre os seus dados. Ligar a sincronização geral do aplicativo (Configurações → Sincronização e Backup) **não** liga este envio: sem o segundo sim, o registro não sai do aparelho, em nenhum plano e com a sincronização geral ligada ou não. Recusar qualquer um dos dois não apaga nada nem impede o resto do aplicativo.
...
- **Apagar tudo.** Configurações → Privacidade apaga os dias registrados e as páginas deles no Grimório neste aparelho; o botão de apagar a cópia da nuvem, na mesma tela, apaga o que está no servidor; e excluir a conta (Perfil → Gerenciar Seus Dados) remove os dois lados.
